...
def connect_to_db_locally():
    tunnel = sshtunnel.SSHTunnelForwarder(
        ...
        ssh_username='danielonline', 
        ssh_password='pa_mysql_pass',
        remote_bind_address=(
            'danielonline.mysql.pythonanywhere-services.com', 3306)
    )
...
def get_local_conn_string():
    ...
    return 'mysql://danielonline:{}@127.0.0.1:{}/danielonline$test'.format(
        os.environ.get('ACCOUNT_PASS'), tunnel.local_bind_port)


def get_prod_conn_string():
    return "mysql+mysqlconnector://{username}:{password}@{hostname}/{databasename}".format(
        username="danielonline",
        password=os.environ.get('ACCOUNT_PASS'),
        hostname="danielonline.mysql.pythonanywhere-services.com'",
        databasename="danielonline$production"
    )
